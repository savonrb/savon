...
In case your service requires the elements to be in a specific order (and not working with Ruby 1.9), you have two options. The first is to specify your body as an XML String. The second is to add an "@inorder" key containing an Array of elements in the specific order to your Hash. Here's an example:

  response = client.add_user do |soap|
    soap.body = { :user => { :name => "Lucy", :id => 666, :@inorder => [:id, :name] } }
  end
